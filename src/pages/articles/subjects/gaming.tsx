...
export default function GamingArticlesPage() {
  const [selectedProvider, setSelectedProvider] = useState<RSSFeedProvider>(
    () => {
      const stored = localStorage.getItem("GamingArticlesSelectedProvider");
      if (stored) {
        const provider = RSS_PROVIDERS.find((p) => p.id === Number(stored));
        if (provider) return provider;
      }
      return RSS_PROVIDERS.find((p) => p.subject === "gaming")!;
    }
  );
  ...
  const handleProviderChange = (id: string) => {
    ...
    localStorage.setItem("GamingArticlesSelectedProvider", id);
  };
            ...
            <SelectContent>
              <SelectGroup>
                {RSS_PROVIDERS.filter(
                  (provider) => provider.subject === "gaming"
                ).map((provider) => (
